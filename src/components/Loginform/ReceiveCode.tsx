...
const schema = z.object({    
  inputs: z.array(    
    z.string().length(1, 'فقط یک عدد مجاز است').regex(/^\d$/, 'فقط اعداد مجاز هستند')    
    ...
    <div className="flex items-center justify-center min-h-screen relative">    
      ...
      <div className="bg-white shadow-[#E3EBE9] rounded-xl shadow-md max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl h-auto z-10 mt-[57px] mb-[100px]">    
        ...
        <div className="flex flex-col items-center justify-center mx-4 sm:mx-10 md:mx-20 lg:mx-24 mt-[15px]">    
          <h1 className="font-iransansNumber font-medium text-xl leading-[30px] mt-[15px]">کد را وارد کنید</h1>    
          <div className='flex flex-row justify-center items-center mt-[18px]'>    
            <h4 className="font-iransansNumber font-ultraLight text-[10px] leading-5 text-right"> شماره موبایل را اشتباه وارد کرده اید؟ اصلاح کنید</h4>    
            <Link href={"/Loginform"}>    
              <img src="/image/LoginForm/Edit.png" alt="Edit" className='w-[18px] h-[18px] mr-14' />    
            </Link>    
          </div>    

          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col items-end mt-[15px] font-iransansNumber">    
            {errorMessage && (    
            ...
            {successMessage && (    
              <div className="p-2 w-56 bg-white shadow-md shadow-slate-200 text-xs font-light text-green-600 rounded-md text-right ml-12">{successMessage}    
              </div>)}    
            <Controller    
              name="inputs"    
              control={control}    
            ...
            <div className='flex flex-col mb-[64px] mt-[15px]'>    
              <button type="submit" className="btn btn-success text-white text-sm leading-5 font-iransansNumber w-full sm:w-[271px]  h-[42px] rounded-md mt-[15px]">ورود</button>    
